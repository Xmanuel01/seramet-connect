...
export class DarajaMpesaProvider implements PaymentProvider {
  ...
  async createPrompt(intent: PaymentIntent, customerPhone: string): Promise<ProviderResult> {
    ...
    return {
      ...
      customerMessage: raw.ResponseDescription,
      raw,
    };
  }
  ...
  parseWebhook(payload: unknown): ProviderWebhookResult {
    ...
    return {
      ...
      reference: String(receipt ?? callback.CheckoutRequestID ?? callback.MerchantRequestID),
      intentReference: String(callback.CheckoutRequestID ?? callback.MerchantRequestID ?? receipt),
...
export function createPaymentIntentForProvider(
  ...
  return TransactionEngine.createPaymentIntent(state, invoiceId, {
    amount,
    method,
    provider: providerName(method),
    createdBy,
    customerPhone,
  });
}

function providerName(method: PaymentMethod) {
  if (method === "MPESA_PROMPT" || method === "MPESA_QR" || method === "MPESA_TILL_MANUAL")
    return "M-Pesa";
  if (method === "CARD") return "Card Acquirer";
  if (method === "BANK_TRANSFER") return "Bank Feed";
