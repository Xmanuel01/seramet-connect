...
const rows: LeaveRequest[] = emptyRecords();
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Metric label="Days requested" value={scopedRows.reduce((sum, row) => sum + row.days, 0)} />
        <Metric
          label="Approved this month"
          value={scopedRows.filter((row) => row.status === "Approved").length}
        />
        <Metric label="Coverage risk days" value={0} invert />
      </div>
      ...
      <Panel className="mt-4">
        <PanelHead
          ...
          sub="HR approval applies where the leave policy requires it"
        />
