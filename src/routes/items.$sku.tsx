...
export const Route = createFileRoute("/items/$sku")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Item overview, stock by warehouse, pricing, recipe usage, suppliers and movement history." },
      { property: "og:title", content: "Item Detail — Seramet" },
      { property: "og:description", content: "Stock, pricing, suppliers and movement for a single item." },
...
const movement = [
  { date: "13 Aug 09:12", type: "Issue to kitchen", qty: "-4.0 kg", by: "Kelvin M.", ref: "ISS-2211" },
  { date: "12 Aug 16:40", type: "Goods received", qty: "+30.0 kg", by: "Kelvin M.", ref: "GRN-0912" },
...
function ItemDetail() {
  ...
  return (
    <AppShell
      ...
      actions={<><Btn>Adjust stock</Btn><Btn>Transfer</Btn><Btn variant="primary">Edit item</Btn></>}
    >
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
              <Panel>
                {tab === "Movement" || tab === "History" ? (
                  <>
                    ...
                    <DataTable cols={["Date", "Type", { l: "Quantity", r: true }, "User", "Reference"]}>
                      {movement.map((m) => (
                        <tr key={m.ref} className="hover:bg-secondary/50">
                          ...
                          <TD className={"num text-right font-semibold " + (m.qty.startsWith("+") ? "text-success" : "text-danger")}>{m.qty}</TD>
                          <TD>{m.by}</TD>
                  ...
                  <>
                    ...
                    <DataTable cols={["Date", "Supplier", { l: "Unit cost", r: true }, { l: "Change", r: true }]}>
                      {[["12 Aug", "Main Meat Supplier", 620, "+12%"], ["28 Jul", "Main Meat Supplier", 554, "+3%"], ["09 Jul", "Main Meat Supplier", 538, "0%"]].map((r) => (
                        <tr key={String(r[0])}>
                  ...
                  <>
                    ...
                    <DataTable cols={["Dish", { l: "Per portion", r: true }, { l: "Portions sold (7d)", r: true }, { l: "Usage", r: true }]}>
                      {[["Beef Dry Fry", "0.22 kg", 62, "13.6 kg"], ["Beef Pilau", "0.18 kg", 74, "13.3 kg"], ["Beef Samosa", "0.05 kg", 140, "7.0 kg"]].map((r) => (
                        <tr key={String(r[0])}>
                  ...
                  <>
                    ...
                    <DataTable cols={["Supplier", "Lead time", { l: "Last price", r: true }, "On-time", "Status"]}>
                      {[["Main Meat Supplier", "1 day", 620, "88%", "Attention"], ["Samwest", "2 days", 648, "96%", "Healthy"]].map((r) => (
                        <tr key={String(r[0])}>
                          <TD className="font-semibold">{r[0]}</TD>
                          <TD className="text-muted-foreground">{r[1]}</TD>
                          <TD className="num text-right">{ksh(Number(r[2]))}</TD>
                          <TD className="num">{r[3]}</TD>
                          <TD><Status>{String(r[4])}</Status></TD>
                        </tr>
                      ))}
                    </DataTable>
                  </>
                  ...
                  <>
                    ...
                    <DataTable cols={["Warehouse", { l: "Available", r: true }, { l: "Reserved", r: true }, { l: "Expected", r: true }, { l: "PAR", r: true }, "Status"]}>
                      {stockByStore.map((s) => (
                        <tr key={s.store} className="hover:bg-secondary/50">
                          <TD className="font-semibold">{s.store}</TD>
                          <TD className="num text-right">{s.available} {item.unit}</TD>
                          <TD className="num text-right text-muted-foreground">{s.reserved} {item.unit}</TD>
                          <TD className="num text-right text-muted-foreground">{s.expected} {item.unit}</TD>
                          <TD className="num text-right">{s.par} {item.unit}</TD>
                          <TD><Status>{s.available < s.par * 0.5 ? "Critical" : s.available < s.par ? "Low" : "Healthy"}</Status></TD>
                        </tr>
                      ))}
                    </DataTable>
                  </>
              ...
              <Panel>
                ...
                <dl className="divide-y divide-border text-[13px]">
                  {[["SKU", item.sku], ["Category", item.cat], ["Unit", item.unit], ["Preferred supplier", item.supplier], ["Reorder level", `${Math.round(item.par * 0.5)} ${item.unit}`], ["Suggested order", `${Math.max(0, Math.round(item.par - item.stock))} ${item.unit}`], ["Last counted", "11 Aug 2026"]].map(([k, v]) => (
                ...
                <div className="border-t border-border p-3">
                  <Btn variant="primary" className="w-full">Add to purchase order</Btn>
                </div>
              </Panel>
            </div>
          )}
